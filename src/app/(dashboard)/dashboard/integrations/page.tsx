...
export default function IntegrationsPage() {
  ...
  let logsIntervalRef = useRef<NodeJS.Timeout | null>(null);
  ...
  // Poll logs every 3 seconds once loaded
  useEffect(() => {
    if (isLoaded) {
      fetchLogs();
        ...
        <div className="bg-card border border-border rounded-xl p-6 md:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-foreground">Real-time Webhook Logs</h2>
                <span className="text-xs text-muted-foreground mt-1 inline-block">Testing & Debugging</span>
              </div>
            </div>
            <button
              onClick={() => fetchLogs()}
              className="px-3 py-1.5 text-xs font-bold bg-secondary text-foreground rounded hover:bg-secondary/80 transition-colors"
            >
              Refresh
            </button>
          </div>
          ...
          <div className="mt-4 border border-border rounded-lg bg-background overflow-hidden flex flex-col h-[300px]">
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              {logs.map((log) => (
                <div key={log.id} className="text-sm p-3 rounded-lg border border-border/50 bg-secondary/20">
                  <div className="flex items-center justify-between mb-2">
                    <span className={clsx("font-bold text-xs uppercase px-2 py-0.5 rounded", 
                      log.provider === 'saweria' ? "bg-purple-500/20 text-purple-400" : "bg-blue-500/20 text-blue-400"
                    )}>
                      {log.provider}
                    </span>
                  ...
                  <pre className="text-xs text-muted-foreground overflow-x-auto p-2 bg-background rounded border border-border/30">
              ...
              {logs.length === 0 && (
                <div className="text-center text-muted-foreground text-sm italic my-auto">
                  Waiting for webhooks... Try sending a test ping from Saweria or Bagibagi.
                </div>
              )}
            </div>
          </div>
